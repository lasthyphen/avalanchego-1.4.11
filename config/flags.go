...
func addNodeFlags(fs *flag.FlagSet) {
	...
	// DJTX fees
	fs.Uint64(TxFeeKey, units.MilliDjtx, "Transaction fee, in nDJTX")
	fs.Uint64(CreationTxFeeKey, units.MilliDjtx, "Transaction fee, in nDJTX, for transactions that create new state")
	...
	fs.Uint64(MinValidatorStakeKey, 2*units.KiloDjtx, "Minimum stake, in nDJTX, required to validate the primary network")
	// Maximum Stake that can be staked and delegated to a validator on the Primary Network
	fs.Uint64(MaxValidatorStakeKey, 3*units.MegaDjtx, "Maximum stake, in nDJTX, that can be placed on a validator on the primary network")
	// Minimum Stake that can be delegated on the Primary Network
	fs.Uint64(MinDelegatorStakeKey, 25*units.Djtx, "Minimum stake, in nDJTX, that can be delegated on the primary network")
