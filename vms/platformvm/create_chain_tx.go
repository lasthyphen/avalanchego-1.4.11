...
func (tx *UnsignedCreateChainTx) SemanticVerify(
	...
	if err := tx.Verify(vm.ctx, vm.codec, vm.CreationTxFee, vm.ctx.DJTXAssetID); err != nil {
	...
	if err := vm.semanticVerifySpend(vs, tx, tx.Ins, tx.Outs, baseTxCreds, vm.CreationTxFee, vm.ctx.DJTXAssetID); err != nil {
		return nil, err
	}
...
func (vm *VM) newCreateChainTx(
	...
	return tx, utx.Verify(vm.ctx, vm.codec, vm.CreationTxFee, vm.ctx.DJTXAssetID)
}
