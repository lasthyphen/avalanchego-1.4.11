...
func (w *Wallet) RemoveUTXO(utxoID ids.ID) {
	...
	newBalance := w.balance[assetID] - utxo.Out.(djtx.TransferableOut).Amount()
	if newBalance == 0 {
...
func (w *Wallet) CreateTx(assetID ids.ID, amount uint64, destAddr ids.ShortID) (*avm.Tx, error) {
	...
	djtx.SortTransferableOutputs(outs, w.codec)

	tx := &avm.Tx{UnsignedTx: &avm.BaseTx{BaseTx: djtx.BaseTx{
